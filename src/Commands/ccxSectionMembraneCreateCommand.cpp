...
ccxSectionMembraneCreateCommand::ccxSectionMembraneCreateCommand()
{}

ccxSectionMembraneCreateCommand::~ccxSectionMembraneCreateCommand()
{}

std::vector<std::string> ccxSectionMembraneCreateCommand::get_syntax()
{
  ...
  for (size_t syn_i = 1; syn_i < 6; syn_i++)
  {
    ...
    syntax.append("create section membrane block ");
...
std::vector<std::string> ccxSectionMembraneCreateCommand::get_syntax_help()
{
  ...
  help[0] = "ccx create section membrane block <block id> material <material_name> [orientation <orientation_name>] [thickness <thickness>] [offset <offset>]";
  help[1]=" ";
  help[2]=" ";
  help[3]=" ";
  help[4]=" ";
...
std::vector<std::string> ccxSectionMembraneCreateCommand::get_help()
{
  std::vector<std::string> help;
  return help;
}

bool ccxSectionMembraneCreateCommand::execute(CubitCommandData &data)
{
  ...
  for (size_t i = 0; i < block_ids.size(); i++)
  {    
    if (!ccx_iface.create_section("MEMBRANE",block_ids[i],material_name,options))
    {
      PRINT_ERROR("Failed!\n");
    } 
  }  
