...
ccxFieldOutputNodeModifyCommand::ccxFieldOutputNodeModifyCommand()
{}

ccxFieldOutputNodeModifyCommand::~ccxFieldOutputNodeModifyCommand()
{}

std::vector<std::string> ccxFieldOutputNodeModifyCommand::get_syntax()
{
  ...
  keys_list = ccx_iface.get_fieldoutput_node_keys();
...
std::vector<std::string> ccxFieldOutputNodeModifyCommand::get_syntax_help()
{
  CalculiXCoreInterface ccx_iface;
  std::vector<std::string> keys_list;
...
std::vector<std::string> ccxFieldOutputNodeModifyCommand::get_help()
{
  std::vector<std::string> help;
  return help;
}

bool ccxFieldOutputNodeModifyCommand::execute(CubitCommandData &data)
{
  CalculiXCoreInterface ccx_iface;
  ...
  if (!ccx_iface.modify_fieldoutput(output_id, 1, options, options_marker))
  {
    output = "Failed!\n";
    PRINT_ERROR(output.c_str());
  }
