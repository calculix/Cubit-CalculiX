...
bool ccxExportCommand::write_file(std::ofstream& output_file, MeshExportInterface *iface)
{
  ...
  result = write_connectivity(output_file, iface);
...
bool ccxExportCommand::write_connectivity(std::ofstream& output_file,MeshExportInterface *iface)
{
  ...
  for (size_t i = 0; i < blocks.size(); i++)
  {
    ...
    while( (num_elems = iface->get_block_elements(start_index, buf_size, block, element_type, handles)) > 0)
    {
      ...
      if (element_type[0] != 0) {
        ...
        {
          /*
          TYPE=SPHERE type id:0
          TYPE=BAR type id:1
          TYPE=BAR2 type id:2
          TYPE=BAR3 type id:3
          TYPE=BEAM type id:4
          TYPE=BEAM2 type id:5
          TYPE=BEAM3 type id:6
          TYPE=TRUSS type id:7
          TYPE=TRUSS2 type id:8
          TYPE=TRUSS3 type id:9
          TYPE=SPRING type id:10
          TYPE=TRI type id:11
          TYPE=TRI3 type id:12
          TYPE=TRI6 type id:13
          TYPE=TRI7 type id:14
          TYPE=TRISHELL type id:15
          TYPE=TRISHELL3 type id:16
          TYPE=TRISHELL6 type id:17
          TYPE=TRISHELL7 type id:18
          TYPE=SHELL type id:19
          TYPE=SHELL4 type id:20
          TYPE=SHELL8 type id:21
          TYPE=SHELL9 type id:22
          TYPE=QUAD type id:23
          TYPE=QUAD4 type id:24
          TYPE=QUAD5 type id:25
          TYPE=QUAD8 type id:26
          TYPE=QUAD9 type id:27
          TYPE=Tetra type id:28
          TYPE=Tetra4 type id:29
          TYPE=Tetra8 type id:30
          TYPE=Tetra10 type id:31
          TYPE=Tetra14 type id:32
          TYPE=Tetra15 type id:33
          TYPE=HEX type id:39
          TYPE=HEX8 type id:40
          TYPE=HEX9 type id:41
          TYPE=HEX20 type id:42
          TYPE=HEX27 type id:43
          */

          // choose right element type
          if (element_type[0] == 1) {
            // BAR
            element_type_name = "B21";
          } else if (element_type[0] == 2) {
            // BAR2
            element_type_name = "B21";
          } else if (element_type[0] == 3) {
            // BAR3
            element_type_name = iface->get_element_type_name(element_type[0]) + " type id:" + std::to_string(element_type[0]);
          } else if (element_type[0] == 4) {
            // BEAM
            element_type_name = "B31";
          } else if (element_type[0] == 5) {
            // BEAM2
            element_type_name = "B31";
          } else if (element_type[0] == 6) {
            // BEAM3
            element_type_name = "B32";
          } else if (element_type[0] == 7) {
            // TRUSS
            element_type_name = "T3D2";
