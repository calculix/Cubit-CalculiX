...
public:
  ...
  std::vector<std::vector<double>> cross_product_matrix(std::vector<double> vec_a, std::vector<double> vec_b);
  ...
  bool draw_arrow(std::vector<double> start_point, std::vector<double> direction, bool inverse_head, std::string color, double size); //draw an arrow from a center point, vector, color, size
