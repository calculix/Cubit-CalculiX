...
std::string CoreLoadsGravity::print_data()
{
  ...
  str_return.append("load_id, OP MODE, amplitude_id, time_delay_id, block_id, direction_id \n");

  for (size_t i = 0; i < loads_data.size(); i++)
  {
    str_return.append(std::to_string(loads_data[i][0]) + " " + std::to_string(loads_data[i][1]) + " " + std::to_string(loads_data[i][2]) + " " + std::to_string(loads_data[i][3]) + " " + std::to_string(loads_data[i][4]) + " " + std::to_string(loads_data[i][5]) + " \n");
  }
