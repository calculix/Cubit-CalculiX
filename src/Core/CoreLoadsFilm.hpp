#ifndef CORELOADSFILM_HPP
#define CORELOADSFILM_HPP
...
public:
  ...
  // loads_data[0][7] film_amplitude_id
  // loads_data[0][8] film_time_delay_id
  ...
  std::vector<std::vector<std::string>> film_time_delay_data;
  // film_time_delay_data[0][0] film_time_delay_id
  // film_time_delay_data[0][1] film_time_delay_value
  ...
  bool add_load(int load_id, int op_mode, int amplitude_id, int time_delay_id, int sideset_id, int temperature_id, int coefficient_id,int film_amplitude_id, int film_time_delay_id, int name_id); // adds new load to loads_data
  ...
  bool add_film_time_delay(std::string time_delay_id, std::string time_delay_value); // adds new time delay to time_delay_data
  ...
  int  get_film_time_delay_data_id_from_film_time_delay_id(int film_time_delay_id); // searches for the film_time_delay_id in the film_time_delay_data and returns the indices or -1 if it fails
