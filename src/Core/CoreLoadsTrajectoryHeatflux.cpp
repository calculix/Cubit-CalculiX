    ...
    */
    ...
    for (size_t ii = 0; ii < trajectory_face_ids.size(); ii++)
    {
      if (trajectory_face_ids[ii].size()>0)
      {
        ...
        ccx_iface->silent_cmd("ccx create amplitude name \"Trajectory_" + std::to_string(loads_data[i][0]) + "_" + name + "_"  + std::to_string(trajectory_face_ids[ii][0]) + "\" time_amplitude " + amplitude + " totaltime ");
