...
StepsBCsDisplacementsTree::StepsBCsDisplacementsTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

StepsBCsDisplacementsTree::~StepsBCsDisplacementsTree()
{
}

void StepsBCsDisplacementsTree::initialize(int step_id_init)
{
  if(isInitialized)
    return;
  ...
  this->setIcon(0,ccx_iface->getIcon2("StepsBCsDisplacementsTree"));
  isInitialized = true;
  step_id = step_id_init;
}

void StepsBCsDisplacementsTree::update()
{
  std::vector<std::vector<std::string>> bcs_tree_data;
  bcs_tree_data = ccx_iface->get_steps_bcsdisplacements_tree_data(step_id); 
...
void StepsBCsDisplacementsTree::clear()
{
...
void StepsBCsDisplacementsTree::addBC(QString bc_id, QString bc_name)
{
  ...
  BCTreeChild->setIcon(0,ccx_iface->getIcon2("StepsBCsDisplacementsTree"));

  this->addChild(BCTreeChild);
}

void StepsBCsDisplacementsTree::removeBC(QTreeWidgetItem *bc)
{
  this->removeChild(bc);
  delete bc;
}

int StepsBCsDisplacementsTree::get_child_id(std::string bc_id)
{
  int int_return;
  int_return = -1;
