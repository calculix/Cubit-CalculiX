...
StepsLoadsFilmTree::StepsLoadsFilmTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

StepsLoadsFilmTree::~StepsLoadsFilmTree()
{
}

void StepsLoadsFilmTree::initialize(int step_id_init)
{
  if(isInitialized)
    return;
  ...
  this->setIcon(0,ccx_iface->getIcon2("StepsLoadsFilmTree"));  
  isInitialized = true;
  step_id = step_id_init;
}

void StepsLoadsFilmTree::update()
{
  std::vector<std::vector<std::string>> loads_tree_data;
  loads_tree_data = ccx_iface->get_steps_loadsfilm_tree_data(step_id); 
...
void StepsLoadsFilmTree::addLoad(QString load_id, QString load_name)
{
  ...
  LoadTreeChild->setIcon(0,ccx_iface->getIcon2("StepsLoadsFilmTree"));  

  this->addChild(LoadTreeChild);
}

void StepsLoadsFilmTree::removeLoad(QTreeWidgetItem *load)
{
  this->removeChild(load);
  delete load;
}

int StepsLoadsFilmTree::get_child_id(std::string load_id)
{
  int int_return;
  int_return = -1;
