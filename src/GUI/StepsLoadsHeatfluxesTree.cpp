...
StepsLoadsHeatfluxesTree::StepsLoadsHeatfluxesTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

StepsLoadsHeatfluxesTree::~StepsLoadsHeatfluxesTree()
{
}

void StepsLoadsHeatfluxesTree::initialize(int step_id_init)
{
  if(isInitialized)
    return;

  QTreeWidgetItem::setText(0,"Heatfluxes");
  CalculiXCoreInterface *ccx_iface = new CalculiXCoreInterface();
  this->setIcon(0,ccx_iface->getIcon2("StepsLoadsHeatfluxesTree"));  
  isInitialized = true;
  step_id = step_id_init;
}

void StepsLoadsHeatfluxesTree::update()
{
  std::vector<std::vector<std::string>> loads_tree_data;
  loads_tree_data = ccx_iface->get_steps_loadsheatfluxes_tree_data(step_id); 
...
void StepsLoadsHeatfluxesTree::clear()
{
...
void StepsLoadsHeatfluxesTree::addLoad(QString load_id, QString load_name)
{
  ...
  LoadTreeChild->setIcon(0,ccx_iface->getIcon2("StepsLoadsHeatfluxesTree"));  

  this->addChild(LoadTreeChild);
}

void StepsLoadsHeatfluxesTree::removeLoad(QTreeWidgetItem *load)
{
  this->removeChild(load);
  delete load;
}

int StepsLoadsHeatfluxesTree::get_child_id(std::string load_id)
{
  int int_return;
  int_return = -1;
