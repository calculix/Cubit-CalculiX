...
StepsLoadsCentrifugalTree::StepsLoadsCentrifugalTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

StepsLoadsCentrifugalTree::~StepsLoadsCentrifugalTree()
{
}

void StepsLoadsCentrifugalTree::initialize(int step_id_init)
{
  if(isInitialized)
    return;

  QTreeWidgetItem::setText(0,"Centrifugal");
  CalculiXCoreInterface *ccx_iface = new CalculiXCoreInterface();
  this->setIcon(0,ccx_iface->getIcon2("StepsLoadsCentrifugalTree"));  
  isInitialized = true;
  step_id = step_id_init;
}

void StepsLoadsCentrifugalTree::update()
{
  std::vector<std::vector<std::string>> loads_tree_data;
  loads_tree_data = ccx_iface->get_steps_loadscentrifugal_tree_data(step_id); 
...
void StepsLoadsCentrifugalTree::clear()
{
...
void StepsLoadsCentrifugalTree::addLoad(QString load_id, QString load_name)
{
  ...
  LoadTreeChild->setIcon(0,ccx_iface->getIcon2("StepsLoadsCentrifugalTree"));  

  this->addChild(LoadTreeChild);
}

void StepsLoadsCentrifugalTree::removeLoad(QTreeWidgetItem *load)
{
  this->removeChild(load);
  delete load;
}

int StepsLoadsCentrifugalTree::get_child_id(std::string load_id)
{
  int int_return;
  int_return = -1;
