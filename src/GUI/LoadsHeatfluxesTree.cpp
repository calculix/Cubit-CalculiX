...
LoadsHeatfluxesTree::LoadsHeatfluxesTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

LoadsHeatfluxesTree::~LoadsHeatfluxesTree()
{
}

void LoadsHeatfluxesTree::initialize()
{
  if(isInitialized)
    return;
  ...
  this->setIcon(0,ccx_iface->getIcon2("LoadsHeatfluxesTree"));
  isInitialized = true;
}

void LoadsHeatfluxesTree::update()
{
  std::vector<std::vector<std::string>> loads_tree_data;
  loads_tree_data = ccx_iface->get_loadsheatfluxes_tree_data(); 
...
void LoadsHeatfluxesTree::clear()
{
...
void LoadsHeatfluxesTree::addLoad(QString load_id, QString load_name)
{
  ...
  LoadTreeChild->setIcon(0,ccx_iface->getIcon2("LoadsHeatfluxesTree"));

  this->addChild(LoadTreeChild);
}

void LoadsHeatfluxesTree::removeLoad(QTreeWidgetItem *load)
{
  this->removeChild(load);
  delete load;
}

int LoadsHeatfluxesTree::get_child_id(std::string load_id)
{
  int int_return;
  int_return = -1;
