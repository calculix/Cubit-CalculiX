...
StepsLoadsGravityTree::StepsLoadsGravityTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

StepsLoadsGravityTree::~StepsLoadsGravityTree()
{
}

void StepsLoadsGravityTree::initialize(int step_id_init)
{
  if(isInitialized)
    return;

  QTreeWidgetItem::setText(0,"Gravity");
  CalculiXCoreInterface *ccx_iface = new CalculiXCoreInterface();
  this->setIcon(0,ccx_iface->getIcon2("StepsLoadsGravityTree"));  
  isInitialized = true;
  step_id = step_id_init;
}

void StepsLoadsGravityTree::update()
{
  std::vector<std::vector<std::string>> loads_tree_data;
  loads_tree_data = ccx_iface->get_steps_loadsgravity_tree_data(step_id); 
...
void StepsLoadsGravityTree::clear()
{
...
void StepsLoadsGravityTree::addLoad(QString load_id, QString load_name)
{
  ...
  LoadTreeChild->setIcon(0,ccx_iface->getIcon2("StepsLoadsGravityTree"));  

  this->addChild(LoadTreeChild);
}

void StepsLoadsGravityTree::removeLoad(QTreeWidgetItem *load)
{
  this->removeChild(load);
  delete load;
}

int StepsLoadsGravityTree::get_child_id(std::string load_id)
{
  int int_return;
  int_return = -1;
