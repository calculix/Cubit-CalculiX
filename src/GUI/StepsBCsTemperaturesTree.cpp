...
StepsBCsTemperaturesTree::StepsBCsTemperaturesTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

StepsBCsTemperaturesTree::~StepsBCsTemperaturesTree()
{
}

void StepsBCsTemperaturesTree::initialize(int step_id_init)
{
  if(isInitialized)
    return;
...
void StepsBCsTemperaturesTree::update()
{
  std::vector<std::vector<std::string>> bcs_tree_data;
  bcs_tree_data = ccx_iface->get_steps_bcstemperatures_tree_data(step_id); 
...
void StepsBCsTemperaturesTree::clear()
{
...
void StepsBCsTemperaturesTree::addBC(QString bc_id, QString bc_name)
{
...
void StepsBCsTemperaturesTree::removeBC(QTreeWidgetItem *bc)
{
  this->removeChild(bc);
  delete bc;
}

int StepsBCsTemperaturesTree::get_child_id(std::string bc_id)
{
  int int_return;
  int_return = -1;
