...
HBCsDisplacementsTree::HBCsDisplacementsTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

HBCsDisplacementsTree::~HBCsDisplacementsTree()
{
}

void HBCsDisplacementsTree::initialize()
{
  if(isInitialized)
    return;
  ...
  this->setIcon(0,ccx_iface->getIcon2("HBCsDisplacementsTree"));
  isInitialized = true;
}

void HBCsDisplacementsTree::update()
{
  std::vector<std::vector<std::string>> bcs_tree_data;
  bcs_tree_data = ccx_iface->get_hbcsdisplacements_tree_data(); 
...
void HBCsDisplacementsTree::clear()
{
...
void HBCsDisplacementsTree::addBC(QString bc_id, QString bc_name)
{
  ...
  BCTreeChild->setIcon(0,ccx_iface->getIcon2("HBCsDisplacementsTree"));

  this->addChild(BCTreeChild);
}

void HBCsDisplacementsTree::removeBC(QTreeWidgetItem *bc)
{
  this->removeChild(bc);
  delete bc;
}

int HBCsDisplacementsTree::get_child_id(std::string bc_id)
{
  int int_return;
  int_return = -1;
