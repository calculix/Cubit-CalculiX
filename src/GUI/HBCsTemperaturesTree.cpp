...
HBCsTemperaturesTree::HBCsTemperaturesTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

HBCsTemperaturesTree::~HBCsTemperaturesTree()
{
}

void HBCsTemperaturesTree::initialize()
{
  if(isInitialized)
    return;
  ...
  this->setIcon(0,ccx_iface->getIcon2("HBCsTemperaturesTree"));
  isInitialized = true;
}

void HBCsTemperaturesTree::update()
{
  std::vector<std::vector<std::string>> bcs_tree_data;
  bcs_tree_data = ccx_iface->get_hbcstemperatures_tree_data(); 
...
void HBCsTemperaturesTree::clear()
{
...
void HBCsTemperaturesTree::addBC(QString bc_id, QString bc_name)
{
  ...
  BCTreeChild->setIcon(0,ccx_iface->getIcon2("HBCsTemperaturesTree"));

  this->addChild(BCTreeChild);
}

void HBCsTemperaturesTree::removeBC(QTreeWidgetItem *bc)
{
  this->removeChild(bc);
  delete bc;
}

int HBCsTemperaturesTree::get_child_id(std::string bc_id)
{
  int int_return;
  int_return = -1;
