...
StepsManagementItem::StepsManagementItem(QTreeWidget* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

StepsManagementItem::~StepsManagementItem()
{
}

void StepsManagementItem::initialize(QString material_id_qstring,QString material_name_qstring)
{
  if(isInitialized)
    return;
...
void StepsManagementItem::update()
{
  MaterialInterface::Material material;
  MaterialInterface::Property prop;
  material = mat_iface->get_material(material_name);
...
int StepsManagementItem::get_properties_data_id_from_group(std::string group)
{
  int return_int = -1;
  for (size_t i = 0; i < group_properties.size(); i++)
  {
...
void StepsManagementItem::setScalarPropertyGUI(std::string group, double prop_scalar)
{
...
double StepsManagementItem::getScalarPropertyGUI(std::string group)
{
  int properties_data_id = get_properties_data_id_from_group(group);
...
void StepsManagementItem::setMatrixPropertyGUI(std::string group, std::vector<std::vector<double>> matrix)
{
  int properties_data_id = get_properties_data_id_from_group(group);
...
std::vector<std::vector<double>> StepsManagementItem::getMatrixPropertyGUI(std::string group)
{
  int properties_data_id = get_properties_data_id_from_group(group);
