...
StepsLoadsBodyHeatfluxTree::StepsLoadsBodyHeatfluxTree(QTreeWidgetItem* parent):
  QTreeWidgetItem (parent),
  isInitialized(false)
{}

StepsLoadsBodyHeatfluxTree::~StepsLoadsBodyHeatfluxTree()
{
}

void StepsLoadsBodyHeatfluxTree::initialize(int step_id_init)
{
  if(isInitialized)
    return;
  ...
  this->setIcon(0,ccx_iface->getIcon2("StepsLoadsBodyHeatfluxTree"));  
  isInitialized = true;
  step_id = step_id_init;
}

void StepsLoadsBodyHeatfluxTree::update()
{
  std::vector<std::vector<std::string>> loads_tree_data;
  loads_tree_data = ccx_iface->get_steps_loadsbodyheatflux_tree_data(step_id); 
...
void StepsLoadsBodyHeatfluxTree::clear()
{
...
void StepsLoadsBodyHeatfluxTree::addLoad(QString load_id, QString load_name)
{
  ...
  LoadTreeChild->setIcon(0,ccx_iface->getIcon2("StepsLoadsBodyHeatfluxTree"));  

  this->addChild(LoadTreeChild);
}

void StepsLoadsBodyHeatfluxTree::removeLoad(QTreeWidgetItem *load)
{
  this->removeChild(load);
  delete load;
}

int StepsLoadsBodyHeatfluxTree::get_child_id(std::string load_id)
{
  int int_return;
  int_return = -1;
